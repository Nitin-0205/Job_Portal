...
export class WorkExperienceService {
  ...
  async delWorkExperience(workExperienceId: string) {
    const exp = await this.worExpRepo.findOne({ where: { workExperienceId } });
    if (!exp) {
      throw new HttpException("Work Experience Not Found !!!", HttpStatus.NOT_FOUND);
    }
    const deleted = await this.worExpRepo.delete({ workExperienceId });
    if (!deleted) {
      throw new HttpException("Work Experience Not Deleted !!!", HttpStatus.INTERNAL_SERVER_ERROR);
    }
